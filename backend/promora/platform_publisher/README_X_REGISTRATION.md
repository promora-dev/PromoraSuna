...
为了解决"下一步"按钮检测和点击的问题，我们添加了基于LLM的按钮检测功能：

1. **GPT-4o Vision集成**：使用GPT-4o Vision模型分析页面截图，精确定位按钮位置
   - 自动检测按钮的精确坐标
   - 识别按钮文本内容和状态
   - 提供最佳点击方式建议

2. **多层次按钮处理策略**：
   - **第一层**：LLM视觉分析定位按钮并直接点击坐标
   - **第二层**：根据LLM建议使用Tab键导航
   - **第三层**：使用传统选择器作为备选方案

3. **智能决策**：LLM不仅检测按钮位置，还能判断按钮状态和最佳交互方式
   - 判断按钮是否可点击
   - 推荐最适合的交互方式（直接点击、键盘导航等）
   - 提供置信度评分
